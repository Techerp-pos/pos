    ...
    const handlePrintReceipt = (orderNumber) => {
        ...
        const itemsData = cart.map((item, index) => {
            ...
            if (isNaN(price) || isNaN(quantity)) {
                ...
                return `Error in item: ${item.name}`; // Fallback if there’s an error
            }

            return `${(index + 1).toString().padEnd(3)} ${item.name.padEnd(20)} ${quantity.toString().padEnd(4)} ${price.toFixed(3).padEnd(6)} ${(price * quantity).toFixed(3)}\n`;
        });
